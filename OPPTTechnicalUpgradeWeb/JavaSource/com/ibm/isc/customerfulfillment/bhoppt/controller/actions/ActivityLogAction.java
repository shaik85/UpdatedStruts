 ...
 * Date 11th Jan 2011
 */
public class ActivityLogAction extends OPPTLongAction 
{
	...
	protected OPPTViewBean executeAction(ActionForm form, OPPTSession session)
		...
		ActivityLogCustomDTO customDto;
		UserMaintenanceActionForm actionForm;

		actionForm = (UserMaintenanceActionForm)form;

		customDto = new ActivityLogCustomDTO();
		setTransactionInformation(customDto, session);
		
		try {
			customDto = getUserManager().getActivityLog(customDto);
			errorReport = customDto.getMessageReport();
			if(null == errorReport){
				viewBean = new ActivityLogViewBean(customDto);
				setForwardName(ForwardNameConstants.ACTIVITY_LOG);  //$NON-NLS-1$
			}
			else{
		...
		session.storeObjectToSession("ActivityLogCustomDTO",customDto);  //$NON-NLS-1$
		return viewBean;
	}
	...
	protected String getActionName() {
		return ActionNameConstants.ACTIVITY_LOG;
	}
