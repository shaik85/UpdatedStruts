 ...
 * @version 5.1A
 */
public class VarianceCheckAction extends OPPTLongAction 
{
	...
	protected OPPTViewBean executeAction(ActionForm form, OPPTSession session)
		...
		VarianceHeaderInformationCustomDTO varianceCustomDto = new VarianceHeaderInformationCustomDTO();
		...
		if(null != contractId && contractId.length == 1){
			...
			setTransactionInformation(varianceCustomDto, session);
			varianceCustomDto.setContractDTO(contractCustomDto);

			try {
				varianceCustomDto = getProposalManager().checkVariance(varianceCustomDto);
				ErrorReport errorReport = varianceCustomDto.getMessageReport();

				if(null == errorReport){
					setInSession(varianceCustomDto, session);
					addDataStoreEntry(session, varianceCustomDto);
					viewBean= new VarianceHeaderInformationViewBean(varianceCustomDto);
	...
	protected String getActionName() {
		return ActionNameConstants.VARIANCE_CHECK;
	}
	...
	private void setInSession(VarianceHeaderInformationCustomDTO varianceCustomDto, OPPTSession session){
		if(null != session){
			session.storeObjectToSession(SessionConstants.VARIANCE_CHECK, varianceCustomDto);
		}
	}
	...
	private void setForwardName(int role){
		switch(role){
			case Constants.PROPOSAL_CREATE_APPROVER: 
								setForwardName(ForwardNameConstants.PAGE_CREATE_APPROVE_CHECK_VARIANCE_STATUS);
								break;

			case Constants.PROPOSAL_ADMINISTRATOR:
			default:
								setForwardName(ForwardNameConstants.PAGE_CHECK_VARIANCE_STATUS);
								break;
		}
	}
