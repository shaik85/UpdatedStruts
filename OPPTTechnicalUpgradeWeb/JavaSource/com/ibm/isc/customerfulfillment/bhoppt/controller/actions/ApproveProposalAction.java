 ...
 * @author thirumalai
 */
public class ApproveProposalAction extends OPPTLongAction {
	/** 
	...
	protected OPPTViewBean executeAction(ActionForm form, OPPTSession session) throws Exception {
		...
		if(null != currentProposal){
			setTransactionInformation(currentProposal, session);
			currentProposal.setConfirmationRequired(true);
			
			try{
				getProposalManager().approveRequest(currentProposal);
				addDataStoreEntry(session, currentProposal);
				setProposalToBeApproved(session, currentProposal);
				viewBean = new ConfirmationViewBean(ConfirmationConstants.APPROVE_PROPOSAL);
			...
			viewBean = handleErrorReport(getErrorReport(ErrorCodeConstants.PA00));
		}
	...
	protected String getActionName() {
		return ActionNameConstants.APPROVE_PROPOSAL;
	}

	private void setProposalToBeApproved(OPPTSession session, ProposalCustomDTO currentProposal){
		if(null != session && null != currentProposal){
			session.storeObjectToSession(SessionConstants.PROPOSAL_TO_BE_APPROVED, currentProposal);
		}
	}
