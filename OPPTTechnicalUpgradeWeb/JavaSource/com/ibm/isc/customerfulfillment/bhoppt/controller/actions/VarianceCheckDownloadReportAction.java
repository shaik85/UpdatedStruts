 ...
 * @author thirumalai
 */
public class VarianceCheckDownloadReportAction extends OPPTAction 
{
	/** 
	...
	protected OPPTViewBean executeAction(ActionForm form, OPPTSession session) throws Exception {
		...
		if(null != actionForm){
			...
			if(null != reportType){
				ProposalCustomDTO selectedProposal = getSelectedProposal(session);
	...
	protected String getActionName() {
		return ActionNameConstants.VARIANCE_CHECK_DOWNLOAD_REPORT;
	}
	...
	private OPPTViewBean getVarianceReport(OPPTSession session, ProposalCustomDTO selectedProposal, boolean priceChange) throws Exception{
		...
		if(null == customDTO || null == customDTO.getReportDTO()) {
			...
			viewBean = new VarianceReportViewBean(customDTO, "variancereportdownload");  //$NON-NLS-1$
			setForwardName(ForwardNameConstants.ACTION_REPORT_GENERATOR);
		}
		return viewBean;
	}
