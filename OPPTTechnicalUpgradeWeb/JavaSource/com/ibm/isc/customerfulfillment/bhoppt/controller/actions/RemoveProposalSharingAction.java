 ...
 * @author thirumalai
 */
public class RemoveProposalSharingAction extends OPPTLongAction 
{
	/** 
		...
		if(null != actionForm)
		{
			...
			if(null != selectedIds)
			{
				if(selectedIds.length == 1)
				{
					...
					{
						ShareProposalCustomDTO customDTO = new ShareProposalCustomDTO();
						...
						try
						{
							...
							if(customDTO.isAccessProvidedByAnotherUser()){
								setForwardName(ForwardNameConstants.ACTION_REFRESH_PROPOSAL_MAINTENANCE);
							}
							else{
								saveShareProposal(session, customDTO);
								addDataStoreEntry(session, customDTO);
								viewBean = new ProposalSharingViewBean(customDTO);
	...
	protected String getActionName() {
		return ActionNameConstants.REMOVE_PROPOSAL_SHARING;
	}
	...
	private void saveShareProposal(OPPTSession session, ShareProposalCustomDTO dto){
		if(null != session && null != dto){
			session.storeObjectToSession(SessionConstants.SHARE_PROPOSAL, dto);
		}
	}
	...
	private void setForwardName(int role){
		switch(role){
			case Constants.PROPOSAL_CREATE_APPROVER: 
								setForwardName(ForwardNameConstants.PAGE_CREATE_APPROVE_REMOVE_PROPOSAL_SHARING);
								break;

			case Constants.PROPOSAL_ADMINISTRATOR:
			default:
								setForwardName(ForwardNameConstants.PAGE_REMOVE_PROPOSAL_SHARING);
								break;
		}
	}
