 ...
 * @author thirumalai
 */
public class RenameProposalAction extends OPPTLongAction 
{
	/** 
	...
	protected OPPTViewBean executeAction(ActionForm form, OPPTSession session)throws Exception 
	{
		...
		if(null != actionForm)
		{
			...
			if(null != selectedIds)
			{
				...
				{
					ProposalCustomDTO customDTO = getProposalToBeRenamed(session, selectedIds[0]);
					if(null != customDTO)
					{
						...
						try{
							...
							setProposalToBeRenamed(session, customDTO);
							viewBean = new NewProposalNameViewBean(NewProposalNameConstants.RENAME_PROPOSAL,NewProposalNameConstants.RENAME_PROPOSAL_DESC);
	...
	protected String getActionName() {
		return ActionNameConstants.RENAME_PROPOSAL;
	}

	private ProposalCustomDTO getProposalToBeRenamed(OPPTSession session, String id) 
	{
	...
	private void setProposalToBeRenamed(OPPTSession session, ProposalCustomDTO proposalToBeRenamed) {
		if(null != session && null != proposalToBeRenamed){
			session.storeObjectToSession(SessionConstants.SESSION_PROPOSAL_TO_BE_RENAMED, proposalToBeRenamed);
		}
	}
