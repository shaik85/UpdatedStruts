 ...
 * @author thirumalai
 */
public class CreateBPUserAction extends OPPTLongAction 
{
	/** 
	...
	protected OPPTViewBean executeAction(ActionForm form, OPPTSession session)
		...
		if(! bpViewBean.getPrimarySalesArea().equals(actionForm.getPrimarySalesOrganization())){
			...
			bpCustomDto = populateCustomDto(bpCustomDto, actionForm);
		
			if(RoleConstants.BUSINESS_PARTNER == Integer.parseInt(actionForm.getRole())){
				try {
					customDto = getUserManager().createBPUser(bpCustomDto);
					errorReport = customDto.getMessageReport();
					if(null == errorReport){
						setForwardName(ForwardNameConstants.ACTION_ADMINISTRATOR);
					}
					else{
						viewBean = new MessageViewBean(ForwardNameConstants.PAGE_BP_USER, true);
						viewBean.setErrorReport(errorReport);
					...
					viewBean = handleServiceException(session, ex.getErrorReport(),ForwardNameConstants.PAGE_BP_USER);
				}
			}
			else{
	...
	protected String getActionName() {
		return ActionNameConstants.CREATE_BPUSER;
	}
