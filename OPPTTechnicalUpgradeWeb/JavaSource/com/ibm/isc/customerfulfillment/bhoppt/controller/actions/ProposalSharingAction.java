 ...
 * @author thirumalai
 */
public class ProposalSharingAction extends OPPTLongAction {
	/** 
			...
			if(null != selectedIds)
			{
				if(selectedIds.length == 1)
				{
					...
					if(null != selectedProposal)
					{
						...
						try
						{
							customDTO = getProposalManager().getEligibleUserToShareProposal(customDTO);
	...
	protected String getActionName() {
		return ActionNameConstants.PROPOSAL_SHARING;
	}
	...
	private void setForwardName(int role){
		switch(role){
			case Constants.PROPOSAL_CREATE_APPROVER: 
								setForwardName(ForwardNameConstants.PAGE_CREATE_APPROVE_PROPOSAL_SHARING);
								break;

			case Constants.PROPOSAL_ADMINISTRATOR:
			default:
								setForwardName(ForwardNameConstants.PAGE_PROPOSAL_SHARING);
								break;
		}
	}
