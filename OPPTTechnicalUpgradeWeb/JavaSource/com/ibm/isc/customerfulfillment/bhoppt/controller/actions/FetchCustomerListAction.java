 ...
 * @version 5.1A
 */
public class FetchCustomerListAction extends OPPTLongAction {
	/** 
	...
	protected OPPTViewBean executeAction(ActionForm form, OPPTSession session) throws Exception {
		...
		if(null != proposalHeader){
			...
			((FetchCustomerListActionForm)form).getRequestParams(customDTO);
			try {
				if(RoleConstants.BUSINESS_PARTNER == customDTO.getRole()){
					...
					//DAD O02
					customDTO = getProposalManager().fetchSiteIDLists(customDTO);
...
//		}
		return true; //DADO02 To enable addFetchSiteID buttion
	}
