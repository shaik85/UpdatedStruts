...
public class _BaseAgreementManagerHome_Stub extends Stub implements BaseAgreementManagerHome {
    
    private static final String[] _type_ids = {
        "RMI:com.ibm.isc.customerfulfillment.bhoppt.service.baseagreement.BaseAgreementManagerHome:0000000000000000", 
    ...
    public BaseAgreementManager create() throws CreateException, RemoteException {
        while(true) {
            if (!Util.isLocal(this)) {
                InputStream in = null;
                try {
                    try {
                        ...
                        in = (InputStream)_invoke(out);
                        return (BaseAgreementManager) in.read_Object(BaseAgreementManager.class);
                    } catch (ApplicationException ex) {
                        in = (InputStream) ex.getInputStream();
                        String id = in.read_string();
                ...
                ServantObject so = _servant_preinvoke("create",com.ibm.isc.customerfulfillment.bhoppt.service.baseagreement.BaseAgreementManagerHome.class);
                if (so == null) {
                    continue;
                }
                try {
                    BaseAgreementManager result = ((com.ibm.isc.customerfulfillment.bhoppt.service.baseagreement.BaseAgreementManagerHome)so.servant).create();
                    return (BaseAgreementManager)Util.copyObject(result,_orb());
                } catch (Throwable ex) {
                    Throwable exCopy = (Throwable)Util.copyObject(ex,_orb());
