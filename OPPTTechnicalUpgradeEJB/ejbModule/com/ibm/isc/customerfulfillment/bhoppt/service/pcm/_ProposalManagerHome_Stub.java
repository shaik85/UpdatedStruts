...
public class _ProposalManagerHome_Stub extends Stub implements ProposalManagerHome {
    
    private static final String[] _type_ids = {
        "RMI:com.ibm.isc.customerfulfillment.bhoppt.service.pcm.ProposalManagerHome:0000000000000000", 
    ...
    public ProposalManager create() throws CreateException, RemoteException {
        while(true) {
            if (!Util.isLocal(this)) {
                InputStream in = null;
                try {
                    try {
                        ...
                        in = (InputStream)_invoke(out);
                        return (ProposalManager) in.read_Object(ProposalManager.class);
                    } catch (ApplicationException ex) {
                        in = (InputStream) ex.getInputStream();
                        String id = in.read_string();
                ...
                ServantObject so = _servant_preinvoke("create",com.ibm.isc.customerfulfillment.bhoppt.service.pcm.ProposalManagerHome.class);
                if (so == null) {
                    continue;
                }
                try {
                    ProposalManager result = ((com.ibm.isc.customerfulfillment.bhoppt.service.pcm.ProposalManagerHome)so.servant).create();
                    return (ProposalManager)Util.copyObject(result,_orb());
                } catch (Throwable ex) {
                    Throwable exCopy = (Throwable)Util.copyObject(ex,_orb());
