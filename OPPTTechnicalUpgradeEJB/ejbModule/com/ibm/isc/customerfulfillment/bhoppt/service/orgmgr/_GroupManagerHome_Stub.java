...
public class _GroupManagerHome_Stub extends Stub implements GroupManagerHome {
    
    private static final String[] _type_ids = {
        "RMI:com.ibm.isc.customerfulfillment.bhoppt.service.orgmgr.GroupManagerHome:0000000000000000", 
    ...
    public GroupManager create() throws CreateException, RemoteException {
        while(true) {
            if (!Util.isLocal(this)) {
                InputStream in = null;
                try {
                    try {
                        ...
                        in = (InputStream)_invoke(out);
                        return (GroupManager) in.read_Object(GroupManager.class);
                    } catch (ApplicationException ex) {
                        in = (InputStream) ex.getInputStream();
                        String id = in.read_string();
                ...
                ServantObject so = _servant_preinvoke("create",com.ibm.isc.customerfulfillment.bhoppt.service.orgmgr.GroupManagerHome.class);
                if (so == null) {
                    continue;
                }
                try {
                    GroupManager result = ((com.ibm.isc.customerfulfillment.bhoppt.service.orgmgr.GroupManagerHome)so.servant).create();
                    return (GroupManager)Util.copyObject(result,_orb());
                } catch (Throwable ex) {
                    Throwable exCopy = (Throwable)Util.copyObject(ex,_orb());
